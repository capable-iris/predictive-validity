...
| A7. **Human phenotype breadth** | Number of distinct HPO terms linked to the gene through human gene-disease annotations; a target-level, indication-agnostic pleiotropy proxy | HPO (`gene_phenotypes` in gb; OMIM/ORPHA annotations) | Count; high breadth is evaluated as a risk signal, not indication-specific support |
