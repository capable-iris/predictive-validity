...
from nelson_tier_io import ingest_directory as ingest_nelson_directory
...
LOCAL_EV = REPO_DATA / "target_evidence"
...
def ingest_evidence_scores(cur):
    ...
    # Cohort-wide v2 LLM adjudications generated by
    # analyses/classifiers/nelson_tier_classify.py. These rows are keyed by
    # database IDs, so no lossy free-text indication remapping is needed. Full
    # source evidence and abstracts remain in the adjacent *.dossiers.jsonl;
    # the score row stores its digest and filename for auditability.
    nelson_paths, nelson_count = ingest_nelson_directory(cur, LOCAL_EV)
    if nelson_count:
        log_ingest(
            cur,
            ", ".join(path.name for path in nelson_paths),
            "preclin.evidence_score",
            nelson_count,
            nelson_count,
            notes="source=nelson_llm; full evidence in dossier sidecars",
        )
    print(f"  evidence (Nelson v2 cohort-wide): {nelson_count}")
